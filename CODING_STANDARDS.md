...
* **Manual, Browser, and Ad-Hoc Checks Are Verification, Not Tests:** Clicking through a UI in a real or automated browser (including Claude-in-Chrome sessions) is a sanity check on top of the suite, never a substitute for it — and so is driving the app by hand through `curl`, `psql`, or a scratch script instead of the UI. Either shape proves nothing to the gate, leaves no regression protection behind, and is not repeatable on demand. Any behavior worth checking this way is worth an automated test — component-level for a pure UI state machine, integration-level where it crosses the network — committed alongside the change, in the same commit as the code it checks. A PR or ticket is not done because "I clicked through it and it worked" or "I curled it and it worked"; it is done when `npm run gate` covers what was clicked or curled. Note: A browser-driven test that is committed to the suite and runs under `npm test`/`npm run gate` — a Playwright script exercising a real page, say — is an automated test like any other, not a manual check in disguise - i.e. if these kind of tests added they are valid tests and not one of verification - this rule only discourages one of checks.
* **Tests Must Be Self-Verifying:** A test asserts its own pass/fail with the test framework's own assertions. It never depends on a person reading command output, a log line, or a screenshot to decide whether the behavior was correct — that is a manual check wearing a test's clothes, and it stops protecting anything the moment nobody rereads it.
