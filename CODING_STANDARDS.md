...
* **Manual and Browser Checks Are Verification, Not Tests:** Clicking through a UI in a real or automated browser (including Claude-in-Chrome sessions) is a sanity check on top of the suite, never a substitute for it. It proves nothing to the gate, leaves no regression protection behind, and is not repeatable on demand. Any behavior worth checking this way is worth an automated test — component-level for a pure UI state machine, integration-level where it crosses the network — committed alongside the change. A PR or ticket is not done because "I clicked through it and it worked"; it is done when `npm run gate` covers what was clicked.
