...
**ADR-0015 settles the shape.** The browser sends the zip to the Book's staging
...
have moved the rest is under half a second. ADR-0015 also refuses `index.html`
...
preview and threw the result away, purely to fill an Unlink count; ADR-0015
...
5× margin, in one invocation. ADR-0015 keeps the preview synchronous and moves only
...
memory rather than CPU, and on HTML size rather than bundle size, so ADR-0015
...
it to the Author before the confirm; ADR-0015 removes it. The durable signal is
ADR-0007's margin, which parks every Unlinked Thread beside the text it used to
hold.
